...
type NavItem = {
  ...
  label: string;
  /**
   * Optional: if set, the item is only shown when user has at least
   * one of these roles.
   */
...
const navSections: NavSection[] = [
...
},
  {
    ...
    items: [
      { href: "/attendance/clock", label: "Clock In/Out" },
      { href: "/attendance/my", label: "My Attendance" },
      {
        href: "/attendance/records",
        label: "Attendance Records",
        requiredRoles: ["HR Manager", "HR Admin", "System Admin", "Payroll Specialist", "Payroll Manager"],
      },
      { href: "/attendance/corrections", label: "Corrections" },
      { href: "/exceptions", label: "Time Exceptions" },
      {
        href: "/reports",
        label: "Reports",
        requiredRoles: ["HR Manager", "HR Admin", "System Admin", "Payroll Specialist", "Payroll Manager"],
  ...
  {
    label: "Time Management Admin",
    items: [
      { href: "/shifts", label: "Shifts" },
      { href: "/shifts/assignments", label: "Shift Assignments" },
      { href: "/shift-types", label: "Shift Types" },
      { href: "/schedule-rules", label: "Schedule Rules" },
      { href: "/holidays", label: "Holidays" },
      { href: "/policies", label: "Policies" },
    ],
    requiredRoles: ["HR Manager", "HR Admin", "System Admin"],
  },
];
...
export function AppShell({
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-slate-100 to-slate-50 text-slate-900">
      <div className="flex min-h-screen">
        {/* Sidebar */}
        <aside className="hidden border-r bg-white md:flex md:w-64 md:flex-col h-screen sticky top-0 shadow-sm">
            ...
            {visibleSections.map((section) => (
              <div key={section.label} className="mb-5">
                ...
                <div className="space-y-1">
                  {section.items
                    .filter((item) => {
                      // Filter items based on requiredRoles if specified
