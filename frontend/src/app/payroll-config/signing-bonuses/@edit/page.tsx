...
function EditSigningBonusForm() {
  ...
  const [formData, setFormData] = useState<UpdateSigningBonusDto>({
    positionName: '',
    amount: 0,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await signingBonusesApi.update(id, formData);
      router.push('/payroll-config/signing-bonuses');
      router.refresh();
    } catch (error: any) {
      console.error('Error updating signing bonus:', error);
      alert(error.message || 'Failed to update signing bonus');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto relative z-[101]">
        <div className="p-6">
          <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">Edit Signing Bonus</h2>
            <button
