...
function EditTerminationBenefitForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const id = searchParams.get('edit');
  const [benefit, setBenefit] = useState<TerminationBenefit | null>(null);
  const [formData, setFormData] = useState<UpdateTerminationBenefitDto>({
    name: '',
    amount: 0,
    terms: '',
  });
    ...
    const loadBenefit = async () => {
      ...
      try {
        ...
        if (data.status !== ConfigStatus.DRAFT) {
          ...
          router.push('/payroll-config/termination-benefits');
        }
      } catch (error) {
        console.error('Error loading termination benefit:', error);
        alert('Failed to load termination benefit');
        router.push('/payroll-config/termination-benefits');
      } finally {
        setFetching(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await terminationBenefitsApi.update(id, formData);
      router.push('/payroll-config/termination-benefits');
      router.refresh();
    } catch (error: any) {
      console.error('Error updating termination benefit:', error);
      alert(error.message || 'Failed to update termination benefit');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto relative z-[101]">
        <div className="p-6">
          <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">Edit Termination Benefit</h2>
            <button
              onClick={() => router.push('/payroll-config/termination-benefits')}
